...
type SystemSetting struct {
	...
	ServerLogoFileID string `json:"server_logo_file_id"` // Server logo file ID
	...
	ICPNumber        string `json:"ICP_number"`          // 备案�?
	FooterContent    string `json:"footer_content"`      // 自定义页脚内�?
	FooterLink       string `json:"footer_link"`         // 自定义页脚链�?
	MetingAPI        string `json:"meting_api"`          // Meting API 地址
	CustomCSS        string `json:"custom_css"`          // 自定�?CSS
...
type S3Setting struct {
	...
	PathPrefix string `json:"path_prefix"` // 存储路径前缀，例�?"uploads/"，方便隔离目�?
...
// PasskeySetting 定义 Passkey(WebAuthn) 配置结构�?
...
type AccessTokenSetting struct {
	...
	Scopes     string `gorm:"type:text" json:"scopes"`                    // scopes �?JSON 字符�?
	...
	LastUsedAt *int64 `json:"last_used_at,omitempty"`                     // 最后一次使用时�?
	CreatedAt  int64  `gorm:"autoCreateTime" json:"created_at"`           // 访问令牌创建时间，Unix 秒级时间�?
}
...
type AgentSetting struct {
	...
	BaseURL    string `json:"base_url"`   // 自定�?API URL（可选）
	Multimodal bool   `json:"multimodal"` // 多模态支持：Chat 检索命中带�?Echo 时，把配图一并递给模型（需所配模型支持视觉）
	// ContextWindow 是模型上下文窗口�?token 数（0=未配置，按保守默认处理）�?
	// 用于区间聚合（年�?月度总结）时的取数预算：窗口越大越倾向「一次塞满全�?Echo」，
...
type SnapshotSchedule struct {
	...
	CronExpression string `json:"cron_expression"` // 定时快照�?Cron 表达�?
}
